...
class Website_DanhmucController extends PublicdetailController {
	...
	public function jsonAction() {
		...
		$offset = ($page - 1) * $rows;
		$jsonObj = json_encode ( Website_Model_Danhmuc::getFetObj ( $sort, $order, $offset, $rows, $this->_ThongtincoquanId ) );
	...
	public function comboAction() {
		...
		try{
			$jsonObj = Website_Model_Danhmuc::getActiveByCoquanId($this->_ThongtincoquanId);
	...
	public function setValue() {
		...
		$data ['TrangChu'] = (isset ( $_REQUEST ['TrangChu'] ) && $_REQUEST ['TrangChu'] == 'on') ? 1 : 0;
		$data ['Alias'] = PublicdetailController::create_slug ( $data ['TieuDe'] );
		$data ['SapXep'] = $_REQUEST ['SapXep'];
	...
	public function addAction() {
		...
		$dup = Website_Model_Danhmuc::dupliObj ( 0, $data ['TieuDe'], $data ['ThongtincoquanId'] );
		if ($dup > 0) {
			$jsonObj ["msg"] = 'Danh mục này đã có trong danh sách!';
			$jsonObj ["success"] = false;
		} else {
			Website_Model_Danhmuc::addObj ( $data );
	...
	public function updateAction() {
		...
		// kiem tra trung lap du lieu
		$dup = Website_Model_Danhmuc::dupliObj ( $id, $data ['TieuDe'], $data ['ThongtincoquanId'] );
		if ($dup > 0) {
			$jsonObj ["msg"] = 'Danh mục này đã có trong danh sách!';
			$jsonObj ["success"] = false;
		} else {
			Website_Model_Danhmuc::updateObj ( $id, $data );
			$jsonObj ["msg"] = 'Cập nhật dữ liệu thành công!';
			$jsonObj ["success"] = true;
		}
		return $this->view->jsonObj = json_encode ( $jsonObj );
		...
		if (count ( $items )) {
			foreach ( $items as $value ) {
				if (isset ( $value ['Id'] ) && $value ['Id'] > 0)
					Website_Model_Danhmuc::delObj ( $value ['Id'] );
			}
			$jsonObj ["msg"] = 'Cập nhật dữ liệu  thành công!';
			$jsonObj ["success"] = true;
		} else {
