 ...
 */
class Websitefront_Model_Lienket {
	protected static $db;
	 ...
	 * Use Zend_Db
	 */
	public static function getLienKet($Loai, $thongtincoquanId) {
		if (self::$db == null) {
			self::$db = Zend_Registry::get ( "db" );
		}
		
		$result = array ();
		$query = self::$db->query ( "SELECT HinhAnh, Link, TieuDe, Id FROM tbl_lienket WHERE Loai=$Loai AND ThongtincoquanId = $thongtincoquanId  AND TrangThai = 1 ORDER BY SapXep" );
		$result = $query->fetchAll ();
		return $result;
	}
	public static function getLientrangchu() {
		if (self::$db == null) {
			self::$db = Zend_Registry::get ( "db" );
		}
	
		$result = array ();
		$query = self::$db->query ( "SELECT Id,Tencoquan FROM tblthongtincoquan where Phuluc=2 union 
				SELECT Id,Tencoquan FROM tblthongtincoquan where Phuluc=0 union
				SELECT Id,Tencoquan FROM tblthongtincoquan where Phuluc=1
				" );
		$result = $query->fetchAll ();
		return $result;
	}
	public static function getImage($Id = 0) {
		return '/public/uploads/lienket/' . $Id . '/thumb.jpg';
	}
	
	public static function checkImage($Id = 0){
		$validator = new Zend_Validate_File_Exists();
		$validator->addDirectory("public/uploads/lienket/$Id");
	...
	public static function getDefaultImage() {
		return '/public/websiteyte/img/trash/8.png';
	}
	...
	public static function getFetObj($sort, $order, $offset, $rows, $thongtincoquanid) {
		...
		$result = array ();
		$query = self::$db->query ( "SELECT COUNT(*) AS Total FROM tbl_lienket WHERE ThongtincoquanId = $thongtincoquanid" );
		$row = $query->fetchAll ();
		$query = self::$db->query ( "SELECT Id, TieuDe, TrangThai, Link
				 FROM tbl_lienket WHERE ThongtincoquanId = $thongtincoquanid ORDER BY $sort $order LIMIT $offset, $rows" );
	...
	public static function addObj($data) {
		...
		$id = self::$db->insert ( 'tbl_lienket', $data );
		return $id;
	}
	public static function updateObj($id, $data) {
		...
		$id = self::$db->update ( 'tbl_danhmuc', $data, 'Id = ' . $id );
		return $id;
	}
	public static function delObj($id) {
		...
		$id = self::$db->delete ( 'tbl_danhmuc', 'Id = ' . $id );
		return $id;
	}
	...
	public static function dupliObj($id, $TieuDe, $thongtincoquanid) {
		...
		$query = self::$db->query ( "SELECT COUNT( * ) AS Total FROM tbl_lienket 
