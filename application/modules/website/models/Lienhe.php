...
class Website_Model_Lienhe {
	...
	public static function getFetObj($sort, $order, $offset, $rows, $thongtincoquanid) {
		...
		$result = array ();
		$query = self::$db->query ( "SELECT COUNT(*) AS Total FROM tbl_lienhe WHERE ThongtincoquanId = $thongtincoquanid" );
		$row = $query->fetchAll ();
		$query = self::$db->query ( "SELECT Id, Ten, Email, ChuDe, NoiDung, NgayTao, DaDoc, 
				if(DaDoc=1, 'Đã Đọc', 'Chưa Đọc') as D
								
				 FROM tbl_lienhe WHERE ThongtincoquanId = $thongtincoquanid ORDER BY $sort $order LIMIT $offset, $rows" );
	...
	public static function delObj($id) {
		...
		$id = self::$db->delete ( 'tbl_lienhe', 'Id = ' . $id );
		return $id;
	}
