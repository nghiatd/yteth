...
class Website_Model_Binhluan {
	...
	public static function getFetObj($sort, $order, $offset, $rows, $thongtincoquanid) {
		...
		$result = array ();
		$query = self::$db->query ( "SELECT COUNT(*) AS Total FROM tbl_binhluan WHERE ThongtincoquanId = $thongtincoquanid" );
		$row = $query->fetchAll ();
		$query = self::$db->query ( "SELECT Id, Ten, Email, ChuDe, NoiDung, NgayTao 
				
								
				 FROM tbl_binhluan WHERE ThongtincoquanId = $thongtincoquanid ORDER BY $sort $order LIMIT $offset, $rows" );
		$result ["total"] = $row [0] ['Total'];
		$result ['rows'] = $query->fetchAll ();
		return $result;
	}
	
	public static function setRead($id = 0, $data){
		if (self::$db == null) {
			self::$db = Zend_Registry::get ( "db" );
		}
		
		$id = self::$db->update ( 'tbl_lienhe', $data, 'Id = ' . $id );
		return $id;
	}
	...
	public static function updateObj($id, $data) {
		...
		$id = self::$db->update ( 'tbl_lienket', $data, 'Id = ' . $id );
		return $id;
	}
	public static function delObj($id) {
		...
		$id = self::$db->delete ( 'tbl_binhluan', 'Id = ' . $id );
		return $id;
	}
	public static function searObj( $ChuDe, $Email, $thongtincoquanid, $sort, $order, $offset, $rows) {
		if (self::$db == null) {
			self::$db = Zend_Registry::get ( "db" );
		}
		...
		if ($ChuDe !='')
			$where = $where . "AND ChuDe LIKE '%$ChuDe%' ";
		//print_r($where);die;
			// Thuc hien cau lenh truy xuat du lieu
			// Lay tong so ban ghi
		$query = self::$db->query ( "SELECT COUNT(*) AS Total FROM tbl_binhluan WHERE ThongtincoquanId = $thongtincoquanid $where" );
		$row = $query->fetchAll ();
		$result ["total"] = $row [0] ['Total'];
		$query = self::$db->query ( "SELECT Id, Ten, Email, ChuDe, NoiDung, NgayTao
				FROM tbl_binhluan WHERE ThongtincoquanId = $thongtincoquanid $where ORDER BY $sort $order LIMIT $offset, $rows" );
