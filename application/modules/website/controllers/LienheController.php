...
class Website_LienheController extends PublicdetailController {
	...
	public function jsonAction() {
		...
		$offset = ($page - 1) * $rows;
		echo json_encode ( Website_Model_Lienhe::getFetObj ( $sort, $order, $offset, $rows, $this->_ThongtincoquanId ) );
	...
	public function updateAction() {
		...
		//Website_Model_Lienhe::setRead($this->getRequest()->getParam('Id'), $data);
	...
	public function setreadAction() {
		...
		if(isset($request['items'])){
			...
			foreach ($request['items'] as $value){
				Website_Model_Lienhe::setRead($value['Id'], $data);
			}
			$jsonObj ["msg"] = 'Cập nhật dữ liệu  thành công!';
			$jsonObj ["success"] = true;
		}
		...
		if (count ( $items )) {
			foreach ( $items as $value ) {
				if (isset ( $value ['Id'] ) && $value ['Id'] > 0)
					Website_Model_Lienhe::delObj ( $value ['Id'] );
			}
			$jsonObj ["msg"] = 'Cập nhật dữ liệu  thành công!';
			$jsonObj ["success"] = true;
		} else {
