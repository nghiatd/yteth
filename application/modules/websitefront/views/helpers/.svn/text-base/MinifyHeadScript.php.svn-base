...
*
*/
class Websitefront_View_Helper_MinifyHeadScript extends Zend_View_Helper_HeadScript {
	 ...
	 * directory
	 * if you modified the directory name at all, you need to let the helper
	 * know
	 * here.
	 * 
	 * @var string
	 ...
	 *
	 * It is important to note that the minified javascript files will be
	 * minified
	...
	protected function _isNeedToMinify($item) {
		return isset ( $item->attributes ['src'] ) && ! empty ( $item->attributes ['src'] ) && preg_match ( '/^https?:\/\//', $item->attributes ['src'] ) == false && ! isset ( $item->attributes ['minify_disabled'] );
	}
