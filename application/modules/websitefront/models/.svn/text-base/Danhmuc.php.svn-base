 ...
 */
class Websitefront_Model_Danhmuc {
	protected static  $db;
	public static $TrangThai = array('0'=>'Không cho phép', '1'=>'Cho phép');
	...
	public  static function getDanhMucObj($TrangChu = 0, $thongtincoquanid=7) {
		if(self::$db==null)
		{
			self::$db = Zend_Registry::get ( "db" );
		}
		...
		$query = self::$db->query ( "SELECT Id, TieuDe , Alias FROM tbl_danhmuc WHERE TrangThai=1 AND TrangChu=$TrangChu AND ThongtincoquanId = $thongtincoquanid Order By SapXep");
				$result = array ();
	...
	public static function getFetObj($sort, $order, $offset, $rows, $thongtincoquanid) {
		...
		$result = array ();
		$query = self::$db->query ( "SELECT COUNT(*) AS Total FROM tbl_danhmuc WHERE ThongtincoquanId = $thongtincoquanid" );
		$row = $query->fetchAll ();
		$query = self::$db->query ( "SELECT Id, TieuDe, TrangThai, SapXep
				 FROM tbl_danhmuc WHERE ThongtincoquanId = $thongtincoquanid ORDER BY $sort $order LIMIT $offset, $rows" );
	...
	public static function addObj($data) {
		...
		$id = self::$db->insert ( 'tbl_danhmuc', $data );
		return $id;
	}
	...
	public static function dupliObj($id, $TieuDe, $Alias, $SapXep, $TrangThai, $thongtincoquanid) {
		if(self::$db==null)
		{
			self::$db = Zend_Registry::get ( "db" );
		}
	...
	public static function getByAlias($Alias = '', $thongtincoquanId=0){
		...
		$result = array();
		$query = self::$db->query("SELECT  Id, TieuDe, Alias FROM tbl_danhmuc WHERE TrangThai = 1 AND ThongtincoquanId = $thongtincoquanId AND Alias = '$Alias'  LIMIT 1");
		$result = $query->fetch();
		return $result;
	}
}