...
class Website_Model_Lienket {
	...
	public static $Loai = array (
			array (
					'Id' => 0,
					'TieuDe' => 'Liên kết phải' 
			),
			array (
					'Id' => 4,
					'TieuDe' => 'Liên kết dưới' 
			),
			array (
					'Id' => 1,
					'TieuDe' => 'Quảng cáo phải' 
			),
			array (
					'Id' => 2,
					'TieuDe' => 'Banner' 
	...
	public static function getFetObj($sort, $order, $offset, $rows, $thongtincoquanid) {
		...
		$row = $query->fetchAll ();
		$query = self::$db->query ( "SELECT Id, TieuDe, TrangThai, Link, Loai,
				if(Loai=1, 'Quảng cáo phải', if(Loai=0, 'Liên kết', if(Loai=2, 'Banner', if(Loai=3, 'Thư viện ảnh', '')))) as L,
	...
	public static function delObj($id) {
		...
		$id = self::$db->delete ( 'tbl_lienket', 'Id = ' . $id );
		return $id;
	}
