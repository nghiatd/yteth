...
class Website_LichcongtacController extends PublicdetailController {
	...
	public function jsonAction() {
		...
		$offset = ($page - 1) * $rows;
		$jsonObj = json_encode ( Website_Model_Lichcongtac::getFetObj ( $sort, $order, $offset, $rows, $this->_ThongtincoquanId ) );
		return $this->view->jsonObj = $jsonObj;
	}

	public function setValue() {
		$data = array ();
		//$data ['SapXep'] = ($_REQUEST ['SapXep']) ? $_REQUEST ['SapXep'] : 0;
		$data ['TieuDe'] = $_REQUEST ['TieuDe'];
// 		$data ['File'] = $_REQUEST ['File'];
// 		$data ['HinhAnh'] = ($_FILES ['HinhAnh'] ['name']) ? 'thumb.jpg' : '';
// 		$data ['Loai'] = $_REQUEST ['Loai'];
	...
	private function getImages($imgDir = '', $File  = '') {
		if (! is_dir ( $imgDir )) {
			mkdir ( $imgDir, 0777 );
		}
		$image = new Zend_Form_Element_File ( 'File' );
		$image->setDestination ( $imgDir );
		$image->addValidator ( 'Extension', false, 'doc, docx, pdf, bmp' );
	...
	public function addAction() {
		...
		$jsonObj = array ();
		$data = $this->setValue ();
		$data ['File'] = $this->changeToSlug($_FILES['File']['name']);
		// kiem tra trung lap du lieu
		$dup = Website_Model_Lichcongtac::dupliObj ( 0, $data ['File'], $data ['ThongtincoquanId'] );
		// kiem tra sap xep
		
		if ($dup > 0) {
			$jsonObj ["msg"] = 'File này đã có trong danh sách!';
			...
			Website_Model_Lichcongtac::addObj ( $data );
			if ($_FILES ['File'] ['name']) {
				$imgDir = APPLICATION_PATH . "/../public/uploads/lichcongtac/" . Website_Model_Lienket::getLastInsertId () . "/";
				$this->getImages ( $imgDir , $data['File']);
			}
	...
	public function changeToSlug($fileName){
		$fileName = explode('.', $fileName);
		if(count($fileName) >= 2)
			$fileName = PublicdetailController::create_slug($fileName[0]) . '.' . $fileName[1];
		else 
			$fileName = '';
		return $fileName;
	...
	public function updateAction() {
		...
		$data = $this->setValue ();
		$countFileName = (strlen($_FILES['File']['name']) > 1)? true:false ;
		
// 		$data ['File'] = (isset($_FILES['File']['name']))? $_FILES['File']['name'] : $this->changeToSlug($_FILES['File']['name']);
		// kiem tra sap xep
		...
		if($countFileName )
			...
			$data ['File'] = $this->getRequest()->getParam('F');
		
		// kiem tra trung lap du lieu
		$dup = Website_Model_Lichcongtac::dupliObj ( $id, $data ['File'], $data ['ThongtincoquanId'] );
		// kiem tra sap xep
		$countFileName = count($_FILES['File']['name']) ;
		if ($dup > 0 &&  $countFileName) {
			$jsonObj ["msg"] = 'Danh mục này đã có trong danh sách!';
			$jsonObj ["success"] = false;
		} else {
			Website_Model_Lichcongtac::updateObj ( $id, $data );
			if ($countFileName) {
				$imgDir = APPLICATION_PATH . "/../public/uploads/lichcongtac/" . $id . "/";
				$this->getImages ( $imgDir , $data['File']);
			}
			$jsonObj ["msg"] = 'Cập nhật dữ liệu thành công!';
			$jsonObj ["success"] = true;
		...
		if (count ( $items )) {
			foreach ( $items as $value ) {
				if (isset ( $value ['Id'] ) && $value ['Id'] > 0)
					Website_Model_Lichcongtac::delObj ( $value ['Id'] );
			}
			$jsonObj ["msg"] = 'Cập nhật dữ liệu  thành công!';
			$jsonObj ["success"] = true;
		} else {
