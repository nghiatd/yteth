...
class Website_Model_Tintuc {
	...
	public static function getFetObj($sort, $order, $offset, $rows, $thongtincoquanid) {
		...
		$row = $query->fetchAll ();
		$query = self::$db->query ( "SELECT Id, TieuDe, MoTa, ChiTiet, NgayTao, Loai, DanhMucId, TrangThai, TinMoi,
				if(TrangThai=1, 'Cho Phép', 'Không cho phép') as T,
				if(TinMoi=1, 'Tin mới', '') as TM,
				if(TinMoi=1, 'on', '') as TinMoi,
				if(Loai=0, 'Tin tức', if(Loai=1, 'Tin mới', if(Loai=2, 'Thông báo', if(Loai=3, 'Lịch làm việc', '')))) as L
	...
	public static function searObj($TieuDe, $TuNgay = null, $DenNgay =  null,  $thongtincoquanid, $sort, $order, $offset, $rows) {
		if (self::$db == null) {
			self::$db = Zend_Registry::get ( "db" );
		}
		...
		$where = '';
		if($TieuDe)
			$where = "  AND TieuDe LIKE '%$TieuDe%'";
		if($TuNgay)
			$where .= "  AND NgayTao > '$TuNgay'";
		
		if(!$DenNgay)
			$where .= "  AND NgayTao < '".date('Y-m-d')."'";
		else 
			$where .= "  AND NgayTao < '".$DenNgay."'";
		
			// Thuc hien cau lenh truy xuat du lieu
			// Lay tong so ban ghi
// 			print_r($where);die;
		$query = self::$db->query ( "SELECT COUNT(Id) AS Total FROM tbl_tintuc WHERE  
				ThongtincoquanId = $thongtincoquanid   $where " );
		...
		$query = self::$db->query ( "SELECT Id, TieuDe, MoTa, ChiTiet, NgayTao, Loai, DanhMucId, TrangThai, TinMoi,
				...
				if(TinMoi=1, 'on', '') as TinMoi,
				if(Loai=0, 'Tin tức', if(Loai=1, 'Tin mới', if(Loai=2, 'Thông báo', if(Loai=3, 'Lịch làm việc', '')))) as L
				FROM tbl_tintuc WHERE ThongtincoquanId = $thongtincoquanid  $where 
