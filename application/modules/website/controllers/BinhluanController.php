...
class Website_BinhluanController extends PublicdetailController {
	...
	public function jsonAction() {
		...
		$offset = ($page - 1) * $rows;
		echo json_encode ( Website_Model_Binhluan::getFetObj ( $sort, $order, $offset, $rows, $this->_ThongtincoquanId ) );
		...
		if (count ( $items )) {
			foreach ( $items as $value ) {
				if (isset ( $value ['Id'] ) && $value ['Id'] > 0)
					Website_Model_Binhluan::delObj ( $value ['Id'] );
			}
			$jsonObj ["msg"] = 'Cập nhật dữ liệu  thành công!';
			$jsonObj ["success"] = true;
		} else {
	...
	public function searAction() {
		...
		$ChuDe = $this->_getParam ( 'ChuDe' );
		$Email = $this->_getParam ( 'Email' );
		
		$jsonObj = json_encode ( Website_Model_Binhluan::searObj ( $ChuDe, $Email, $this->_ThongtincoquanId, $sort, $order, $offset, $rows ) );
		echo  $jsonObj;
	}
