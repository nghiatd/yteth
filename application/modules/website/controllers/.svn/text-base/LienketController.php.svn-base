...
class Website_LienketController extends PublicdetailController {
	...
	public function jsonAction() {
		...
		$offset = ($page - 1) * $rows;
		$jsonObj = json_encode ( Website_Model_Lienket::getFetObj ( $sort, $order, $offset, $rows, $this->_ThongtincoquanId ) );
	...
	public function comboAction() {
		// get Id from url
		$this->_helper->layout ()->disableLayout ();
		@$idChucvu = $this->_getParam ( 'Id' );
		
		$jsonObj = null;
		
		if (isset ( $idChucvu ) && $idChucvu > 0) {
			$jsonObj = json_encode ( Website_Model_Danhmuc::getIdNameById ( $idChucvu ) );
		} else {
			$jsonObj = json_encode ( Website_Model_Danhmuc::getIdName () );
		}
		return $this->view->jsonObj = $jsonObj;
	}
	...
	public function addAction() {
		...
		$dup = Website_Model_Lienket::dupliObj ( 0, $data ['TieuDe'], $data ['ThongtincoquanId'] );
		// kiem tra sap xep
		if (isset ( $data ['SapXep'] ) && ! is_numeric ( $data ['SapXep'] )) {
			$jsonObj ["msg"] = 'Sắp xếp phải là số  thứ tự!';
		...
		if ($dup > 0) {
			...
			Website_Model_Lienket::addObj ( $data );
			if ($_FILES ['HinhAnh'] ['name']) {
				$imgDir = APPLICATION_PATH . "/../public/uploads/lienket/" . Website_Model_Lienket::getLastInsertId () . "/";
	...
	public function updateAction() {
		...
		// kiem tra trung lap du lieu
		$dup = Website_Model_Lienket::dupliObj ( $id, $data ['TieuDe'], $data ['ThongtincoquanId'] );
		// kiem tra sap xep
		if (! is_numeric ( $data ['SapXep'] )) {
			$jsonObj ["msg"] = 'Sắp xếp phải là số  thứ tự!';
			$jsonObj ["success"] = false;
			return $this->view->jsonObj = json_encode ( $jsonObj );
		}
		if ($dup > 0) {
			...
			Website_Model_Lienket::updateObj ( $id, $data );
			if ($_FILES ['HinhAnh'] ['name']) {
				$imgDir = APPLICATION_PATH . "/../public/uploads/lienket/" . $id . "/";
				$this->getImages ( $imgDir );
			}
			$jsonObj ["msg"] = 'Cập nhật dữ liệu thành công!';
			$jsonObj ["success"] = true;
