...
class Website_Model_Danhmuc {
	...
	public static function getActiveByCoquanId($thongtincoquanid){
		if (self::$db == null) {
			self::$db = Zend_Registry::get ( "db" );
		}
		$return = self::$db->query("SELECT Id, TieuDe From tbl_danhmuc WHERE ThongtincoquanId = $thongtincoquanid AND TrangThai=1");
	...
	public static function getFetObj($sort, $order, $offset, $rows, $thongtincoquanid) {
		...
		$row = $query->fetchAll ();
		$query = self::$db->query ( "SELECT Id, TieuDe, TrangThai, SapXep, IF(TrangChu=1, 'on', '') as TrangChu,
	...
	public static function dupliObj($id, $TieuDe, $thongtincoquanid) {
		...
		$query = self::$db->query ( "SELECT COUNT( * ) AS Total FROM tbl_danhmuc 
