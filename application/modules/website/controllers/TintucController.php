...
class Website_TintucController extends PublicdetailController {
	...
	public function searAction() {
		...
		$TieuDe = $this->_getParam ( 'TieuDe' );
		$TuNgay = $this->_getParam ( 'TuNgay' );
		$DenNgay = $this->_getParam ( 'DenNgay' );
		
		
	
		$jsonObj = json_encode ( Website_Model_Tintuc::searObj($TieuDe, $TuNgay, $DenNgay, $this->_ThongtincoquanId,  $sort, $order, $offset, $rows  ));
		echo $jsonObj;
	...
	public function jsonAction() {
		...
		$order = isset ( $_POST ['order'] ) ? strval ( $_POST ['order'] ) : 'desc';
		$offset = ($page - 1) * $rows;
		$jsonObj = json_encode ( Website_Model_Tintuc::getFetObj ( $sort, $order, $offset, $rows, $this->_ThongtincoquanId ) );
		return $this->view->jsonObj = $jsonObj;
	}
	public function getobjbydateAction() {
		...
		$jsonObj = json_encode ( Nhansu_Model_Nhanvien::getFetObjByDate ( $sort, $order, $this->_ThongtincoquanId, $ngaynghi ) );
		return $this->view->jsonObj = $jsonObj;
	}
	public function getobjbyidAction() {
		$this->_helper->layout ()->disableLayout ();
		$id = $this->_getParam ( 'Id' );
		$jsonObj = Nhansu_Model_Nhanvien::getObjById ( $id );
	...
	public function setValue() {
		$data = array ();
		// $data ['Id'] = $_REQUEST ['Id'];
		$data ['TinMoi'] = (isset ( $_REQUEST ['TinMoi'] ) && $_REQUEST ['TinMoi'] == 'on') ? 1 : 0;
		$data ['TieuDe'] = $_REQUEST ['TieuDe'];
		$data ['Alias'] = PublicdetailController::create_slug ( $data ['TieuDe'] );
		$data ['MoTa'] = $_REQUEST ['MoTa'];
		$data ['ChiTiet'] = $_REQUEST ['ChiTiet'];
		$data ['NgayTao'] = date ( 'Y-m-d' );
		$data ['HinhAnh'] = ($_FILES ['HinhAnh'] ['name']) ? 'thumb.jpg' : '';
		$data ['DanhMucId'] = $_REQUEST ['DanhMucId'];
		...
		if ($_FILES ['HinhAnh'] ['name'])
			$data ['HinhAnh'] = 'thumb.jpg';
		return $data;
	}
	private function getImages($imgDir = '') {
		if (! is_dir ( $imgDir )) {
			mkdir ( $imgDir, 0777 );
		}
		$image = new Zend_Form_Element_File ( 'HinhAnh' );
		$image->setDestination ( $imgDir );
		$image->addValidator ( 'Extension', false, 'jpg, png, gif, bmp' );
		$image->addFilter ( 'rename', 'thumb.jpg' );
		$image->receive ();
	}
	public function addAction() {
		...
		if (! $data ['DanhMucId']) {
			$jsonObj ["msg"] = 'Bạn chưa chọn danh mục tin!';
			$jsonObj ["success"] = false;
			return $this->view->jsonObj = json_encode ( $jsonObj );
		}
		
		// kiem tra trung lap du lieu
		$dup = Website_Model_Tintuc::dupliObj ( 0, $data ['TieuDe'], $data ['ThongtincoquanId'] );
		if ($dup > 0) {
			$jsonObj ["msg"] = 'Tin tức này đã có trong danh sách!';
			$jsonObj ["success"] = false;
		} else {
			Website_Model_Tintuc::addObj ( $data );
			if ($_FILES ['HinhAnh'] ['name']) {
				$imgDir = APPLICATION_PATH . "/../public/uploads/news/" . Website_Model_Tintuc::getLastInsertId () . "/";
				$this->getImages ( $imgDir );
			}
	...
	public function loaitinAction() {
		$this->_helper->layout ()->disableLayout ();
		$this->_helper->viewRenderer->setNoRender ( TRUE );
		echo json_encode ( Website_Model_Tintuc::$loaitin );
	}
	public function updateAction() {
		...
		// kiem tra trung lap du lieu
		$dup = Website_Model_Tintuc::dupliObj ( $id, $data ['TieuDe'], $data ['ThongtincoquanId'] );
		if ($dup > 0) {
			$jsonObj ["msg"] = 'Tin tức này đã có trong danh sách!';
			$jsonObj ["success"] = false;
		} else {
			Website_Model_Tintuc::updateObj ( $id, $data );
			if ($_FILES ['HinhAnh'] ['name']) {
				$imgDir = APPLICATION_PATH . "/../public/uploads/news/" . $id . "/";
				$this->getImages ( $imgDir );
			}
			$jsonObj ["msg"] = 'Cập nhật dữ liệu thành công!';
			$jsonObj ["success"] = true;
		...
		if (count ( $items )) {
			foreach ( $items as $value ) {
				if (isset ( $value ['Id'] ) && $value ['Id'] > 0)
					Website_Model_Tintuc::delObj ( $value ['Id'] );
			}
			$jsonObj ["msg"] = 'Cập nhật dữ liệu  thành công!';
			$jsonObj ["success"] = true;
		} else {
